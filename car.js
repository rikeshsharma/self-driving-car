class Car{
    ...
    #assesDamage(roadBorders,traffic){
        ...
        for(let i = 0; i<traffic.length; i++){
            if(traffic[i] && polysIntersect(this.polygon, traffic[i].polygon)){
                return true;
            }
        }
        return false;
    ...
    #createPolygon(){
        ...
        points.push({
            x : this.x - Math.sin(this.angle - alpha)*this.rad,
            y : this.y - Math.cos(this.angle - alpha)*this.rad
        });
        points.push({
            x : this.x - Math.sin(this.angle + alpha)*this.rad,
            y : this.y - Math.cos(this.angle + alpha)*this.rad
        ...
        points.push({
            x : this.x - Math.sin(Math.PI + this.angle - alpha)*this.rad,
            y : this.y - Math.cos(Math.PI + this.angle - alpha)*this.rad
        });
        points.push({
            x : this.x - Math.sin(Math.PI + this.angle + alpha)*this.rad,
            y : this.y - Math.cos(Math.PI + this.angle + alpha)*this.rad
